...
def getCPUorGPUorTPUStrategy():
    ...
    try:
        ...
        print('Running on TPU: Number of accelerators ', strategy.num_replicas_in_sync)
        print(tpu.__dict__)
        ...
        if tpu == None:
            ...
            if gpu == '/device:GPU:0':
                print('Running on GPU: Number of accelerators ', strategy.num_replicas_in_sync)
                # !/opt/bin/nvidia-smi
            else:
                gpu = None
                strategy = tf.distribute.get_strategy();
                print('Running on CPU:Number of accelerators ', strategy.num_replicas_in_sync)
    return strategy
...
class DataUtils:
    ...
    def writeHdf5File(hdfFile, imgArr, imgInfoDf):
        ...
        tCat = [l.encode('ascii', 'ignore') for l in imgInfoDf['tumorCategory']]
    ...
    def readHdf5Var(hdfFile, varName):
        ...
        Parameters
        ----------
        hdf5File : Name of HDF5 file containing data with full path
    ...
    @staticmethod
    def convertToHdf5(dataPath, dirName, resize):
        dirPath = os.path.join(dataPath,dirName)
        hdfFile = os.path.join(dataPath,(dirName+'_'+str(resize)+'.h5'))
...
def plotImageArr(nRows, nCols, figSize, imgArr, rowLabels=[], colLabels=[], figFile=None):
...
def getShortLabels(labels):
    ...
    """
    if len(labels) > 0:
        return np.array([l if l.startswith('no_') else l.replace('_tumor', '') for l in labels])
    else:
        return labels
...
def plotPixelValueDist(trainArr, testArr, trainInfoDf, testInfoDf, scale='Count', ylim=[], figFile=None):
...
class MyModel:
    ...
    @staticmethod
    def __convBlock2020(x, nFilters=16, kSize=5, kStride=2, poolStride=2, drop=0):
    ...
    @staticmethod
    def __convBlock(x, filtNo, kSize=3, bNorm=False, drop=0, kernelL2=None):
...
def plotAccuracyAndLoss(trainingHistory, saveFn='',varNames=['loss','accuracy']):
...
def getMergedDataSet(dataPath,trainFile,testFile):
    # Merge datasets into a singel dataset
    trainHdf5File = os.path.join(dataPath, trainFile)
    testHdf5File = os.path.join(dataPath, testFile)
    # merge datasets
